...
export default function ContactPage() {
  ...
  return (
    <main className="min-h-screen bg-surface-white pt-28 pb-20">
      <div className="mx-auto max-w-[1200px] px-6 md:px-8 lg:px-12">
        <div className="text-center mb-16">
          <span className="section-label">{CONTACT_PAGE.badge}</span>
          <h1 className="mt-5 font-serif text-heading-1 text-text-primary">{CONTACT_PAGE.title}</h1>
          <p className="mt-4 text-body-lg text-text-muted max-w-xl mx-auto">{CONTACT_PAGE.subtitle}</p>
        </div>
          ...
          <div>
            <div className="space-y-5">
              <div className="flex items-center gap-4">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-sage-50 text-sage-600"><Mail className="h-5 w-5" /></div>
                <div>
                  <p className="text-xs text-text-muted uppercase tracking-wider">Email</p>
                  <a href={`mailto:${CONTACT_PAGE.email}`} className="text-sm font-medium text-text-primary hover:text-sage-600 transition-colors">{CONTACT_PAGE.email}</a>
                </div>
              </div>
              {CONTACT_PAGE.phone && (
                <div className="flex items-center gap-4">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-sage-50 text-sage-600"><Phone className="h-5 w-5" /></div>
                  <div>
                    <p className="text-xs text-text-muted uppercase tracking-wider">Phone</p>
                    <a href={`tel:${CONTACT_PAGE.phone}`} className="text-sm font-medium text-text-primary hover:text-sage-600 transition-colors">{CONTACT_PAGE.phone}</a>
                  </div>
                </div>
              )}
              <div className="flex items-center gap-4">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-sage-50 text-sage-600"><MapPin className="h-5 w-5" /></div>
                <div>
          ...
          <form onSubmit={handleSubmit} className="rounded-3xl border border-border-light bg-white p-8 md:p-10 card-shadow">
            <div className="space-y-6">
              <div>
                ...
                <input type="text" required={CONTACT_FORM.name.required} placeholder={CONTACT_FORM.name.placeholder}
              ...
              <div>
                ...
                <input type="email" required={CONTACT_FORM.email.required} placeholder={CONTACT_FORM.email.placeholder}
                  value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                  className="w-full rounded-xl border border-border-light bg-surface-cream px-4 py-3 text-sm text-text-primary placeholder:text-text-muted/50 focus:outline-none focus:ring-2 focus:ring-sage-400 focus:border-transparent transition-all" />
              </div>
              <div>
                <label className="block text-sm font-medium text-text-primary mb-2">{CONTACT_FORM.message.label}</label>
                <textarea required={CONTACT_FORM.message.required} placeholder={CONTACT_FORM.message.placeholder} rows={5}
